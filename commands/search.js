...
cmd({
        ...
        async(Void, xstro, text) => {
            if (!text) return xstro.reply(`_Name a Series or movie ${tlang().greet}._`);
...
cmd({
        ...
        async(Void, xstro, text) => {
            if (!text) return xstro.reply("Give me location.Baka!!");
...
cmd({
        ...
        async(Void, xstro, text) => {
            if (!text) return xstro.reply("Provide me a sign!")
            try {
    ...
    async(Void, xstro, text) => {
        if (!text) return xstro.reply(`give me a query\n*Example : .google Who is Suhail Tech.*`);
...
cmd({
        ...
        async(Void, xstro, text) => {
            if (!text) return xstro.reply("Provide me a query!")
            if (!text) return reply("Hey bie please tell me for which pic you're looking");
            let name1 = text.split("|")[0]
            let name2 = text.split("|")[1] || `1`
            xstro.reply(`Sending ${name2} image(s) of ${name1} in chat`)
            let nn = name2
            for (let i = 0; i < nn; i++) {
...
cmd({
        ...
        async(Void, xstro, text) => {
            ...
            Void.sendMessage(xstro.chat, { image: { url: random.male }, caption: `Couple Male` }, { quoted: xstro })
            Void.sendMessage(xstro.chat, { image: { url: random.female }, caption: `Couple Female` }, { quoted: xstro })
        }
    )
    //---------------------------------------------------------------------------
cmd({
    ...
    async(Void, xstro, text) => {
        ...
        if (!inputnumber.includes('x')) return xstro.reply('You did not add x\nExample: iswa 9196285162xx')
        xstro.reply(`Searching for WhatsApp account in given range...`)
        ...
        xstro.reply(`${text}${nobio}${nowhatsapp}`)
