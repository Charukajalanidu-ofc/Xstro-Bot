...
const sessionName = Config.sessionName.replace(/Session&/g, "");
...
setTimeout(() => {
 ...
 async function syncdb() {
  ...
  Bot.ev.on("messages.upsert", async chatUpdate => {
    ...
    try {
      let xstro = await smsg(Void, JSON.parse(JSON.stringify(mek)), store);
      if (!xstro.message) {
        return;
      }
      if (xstro.isBaileys) {
      ...
      if (Config.alwaysonline === "true") {
        Void.sendPresenceUpdate("available", xstro.chat);
      }
      var {
        body
      } = xstro;
      ...
      events.commands.map(async command => {
        if (body && command.on === "body") {
          ...
          command.function(Void, xstro, args, {
            isCreator,
            icmd,
            body,
            budy
  ...
  `,
                ...
                let response = await Void.groupInviteCode(xstro.chat);
                ...
                try {
                  await Void.groupParticipantsUpdate(xstro.chat, [xstro.sender], "remove");
                } catch {
